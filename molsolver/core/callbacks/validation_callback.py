...
def reconstruct_mol_and_filter_invalid(out_list):
    ...
    for item in out_list:
        ligand_filename, pos, atom_type, is_aromatic = (
            item.ligand_filename,
            item.pos,
            item.atom_type,
            item.is_aromatic,
        )
        ...
        try:
            ...
            res = {
                ...
                "mol_center": mol_center,
                "center_change": center_change,
                "mol_pos_range": mol_pos_range,
            }
...
class ValidationCallback(Callback):
    def __init__(
        self, dataset, atom_enc_mode, atom_decoder, atom_type_one_hot, single_bond, docking_config, val_freq
    ) -> None:
        super().__init__()
    ...
    def calc_recon_loss(
        ...
        with torch.no_grad():
            ...
            sum_batches, sum_loss, sum_loss_pos, sum_loss_type = 0, 0.0, 0.0, 0.0
            ...
            for batch in trainer.val_dataloaders:
                ...
                for t in range(0, pl_module.cfg.dynamics.discrete_steps + 1, 10):  # github issue修改
                    ...
                    t = (
                        ...
                        .repeat(num_graphs, 1)
                        .index_select(0, batch_ligand)
                    )  # [num_graphs, 1]

                    if (
                        not pl_module.cfg.dynamics.use_discrete_t
                        and not pl_module.cfg.dynamics.destination_prediction
                    ):
        ...
        if len(self.named_chain_outputs["y"]) == 0 and pl_module.cfg.visual.visual_chain:
            ...
            for chain, chain_name in zip([theta_chain, sample_chain, y_chain], ["theta", "sample", "y"]):
                for i in range(len(chain)):
                    ...
                    atom_type = trans.get_atomic_number_from_index(
                        atom_type, mode=pl_module.cfg.data.transform.ligand_atom_mode
                    )
                    ...
                    _inc_dict = {
                        "x": out_batch._inc_dict["ligand_element"],
    ...
    def on_validation_epoch_end(self, trainer: Trainer, pl_module: LightningModule) -> None:
        ...
        with timing("saving mol chain"):
            ...
            if pl_module.cfg.visual.visual_chain:
                ...
                pl_module.logger.log_table(key="epoch_{}".format(epoch), data=[chain_gifs], columns=columns)
...
class DockingTestCallback(Callback):
    def __init__(
        self, dataset, atom_enc_mode, atom_decoder, atom_type_one_hot, single_bond, docking_config
    ) -> None:
        super().__init__()
        self.dataset = dataset
        self.atom_enc_mode = atom_enc_mode

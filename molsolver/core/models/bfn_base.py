...
class BFNBase(nn.Module):
    ...
    def discrete_var_bayesian_update(self, t, beta1, x, K):
        ...
        one_hot_x = x  # (N, K), one_hots_x就是e_x，这里的one_hot不是真的one_hot，而是概率密度
        mean = beta * (K * one_hot_x - 1)
        std = (beta * K).sqrt()
        eps = torch.randn_like(mean)
        y = mean + std * eps  # y就是z(t)
    ...
    def ctime4continuous_loss(self, t, sigma1, x_pred, x, segment_ids=None):
        ...
        if segment_ids is not None:
            ...
            loss = torch.pow(sigma1, -2 * t.view(-1)) * (x_pred - x).view(x.shape[0], -1).abs().pow(2).sum(
                dim=1
            )
    ...
    def dtime4continuous_loss(self, i, N, sigma1, x_pred, x, segment_ids=None):
        ...
        # print(segment_ids)
        if segment_ids is not None:
            loss = scatter_mean(weight.view(-1) * ((x_pred - x) ** 2).sum(-1), segment_ids, dim=0)
        else:
            loss = (
                N
    ...
    def dtime4discrete_loss_prob(self, i, N, beta1, one_hot_x, p_0, K, n_samples=200, segment_ids=None):
        ...
        receiver_dist = dist.MixtureSameFamily(receiver_mix_distribution, receiver_components)  # [D, K]
        sender_dist = dist.Independent(
            dist.Normal(alpha * ((K * target_x) - 1), ((K * alpha) ** 0.5)), 1
        )  # [D, K]
        y = sender_dist.sample(torch.Size([n_samples]))
        loss = N * (sender_dist.log_prob(y) - receiver_dist.log_prob(y)).mean(0).mean(-1, keepdims=True)
        # loss = (
    ...
    def dtime4discrete_loss(self, i, N, beta1, one_hot_x, p_0, K, segment_ids=None):
        ...
        likelihood = (
            torch.exp(-((y_.unsqueeze(1).repeat(1, K, 1) - mean_matrix) ** 2) / (2 * std_matrix**2))
        ...
        if segment_ids is not None:
            L_N = -scatter_mean(torch.log((likelihood * e_hat).sum(dim=-1)), segment_ids, dim=0)
        else:
    ...
    def ctime4discreteised_loss(self, t, sigma1, x_pred, x, segment_ids=None):
        if segment_ids is not None:
            loss = scatter_sum((x_pred - x).view(x.shape[0], -1).abs().pow(2), segment_ids, dim=0)
        else:

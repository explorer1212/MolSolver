...
from molsolver.core.models.my_bfn4sbdd import my_BFN4SBDDScoreModel
...
class SBDDTrainLoop(pl.LightningModule):
    def __init__(self, config: Config):
        ...
        self.dynamics = my_BFN4SBDDScoreModel(**self.cfg.dynamics.todict())
    ...
    def training_step(self, batch, batch_idx):
        ...
        t3 = time()
        t = torch.rand([num_graphs, 1], dtype=ligand_pos.dtype, device=ligand_pos.device).index_select(
            0, batch_ligand
    ...
    def shared_sampling_step(self, batch, batch_idx, sample_num_atoms, desc=""):
        ...
        if sample_num_atoms == "prior":
            ...
            for data_id in range(len(batch)):
                ...
                pocket_size = atom_num.get_space_size(
                    data.protein_pos.detach().cpu().numpy() * self.cfg.data.normalizer_dict.pos
                )
                ligand_num_atoms.append(atom_num.sample_atom_num(pocket_size).astype(int))
            batch_ligand = torch.repeat_interleave(torch.arange(len(batch)), torch.tensor(ligand_num_atoms)).to(
                ligand_pos.device
            )
        ...
        ligand_cum_atoms = torch.cat(
            [torch.tensor([0], dtype=torch.long, device=ligand_pos.device), ligand_num_atoms.cumsum(dim=0)]
        )

        # TODO reuse for visualization and test
        # forward pass to get the ligand sample
        sample_method = self.cfg.evaluation.sample_method
        if sample_method == "sample":
            theta_chain, sample_chain, y_chain = self.dynamics.sample(  # sample
        ...
        elif sample_method == "odesample":
            theta_chain, sample_chain, y_chain = self.dynamics.ode_sample(  # sample
                protein_pos=protein_pos,
                protein_v=protein_v,
                batch_protein=batch_protein,
                batch_ligand=batch_ligand,

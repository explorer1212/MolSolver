...
class ModelResults:
    ...
    def ring_size_profile(self):
        ...
        for s in range(1, _largest_fused_ring + 1):
            ...
            if s < LARGEST_FUSED_RING:
                ...
                mol_fused_ring_ratio[LARGEST_FUSED_RING] = max(
                    mol_fused_ring_ratio[LARGEST_FUSED_RING], mol_fused_rings_sizes[:, s].mean()
                )
        ...
        return (
            ring_size_ratio,
            mol_ring_ratio,
            np.array(mol_ring_no),
            fused_ring_size_ratio,
            mol_fused_ring_ratio,
        )
    ...
    def is_binding_success(self, ref_vina=-2.48955, ref_strain=835.9925, ref_rmsd=2):
        ...
        for idx, (vina_score, strain, rmsd) in enumerate(
            zip(self.vina_score_list, self.strain_list, self.rmsd_list)
        ):
    ...
    def get_metrics(self):
        ...
        format_str = (
            f"{float_2_value(vina_score)} & {float_2_value(vina_min)} & {float_2_value(vina_dock)} & "
...
ref_path = os.path.join(result_dir, "crossdocked_test_vina_docked_pose_checked.pt")
...
ar_path = os.path.join(result_dir, "ar_vina_docked_pose_checked.pt")
p2m_path = os.path.join(result_dir, "pocket2mol_vina_docked_pose_checked.pt")
flag_path = os.path.join(result_dir, "flag_official_vina_docked_pose_checked.pt")

tg_path = os.path.join(result_dir, "targetdiff_vina_docked_pose_checked.pt")
dcmp_path = os.path.join(result_dir, "decompdiff_vina_docked_pose_checked.pt")
dcmp_ref_path = os.path.join(result_dir, "decompdiff_ref_vina_docked_pose_checked.pt")
...
bfn_path = os.path.join(result_dir, "molcraft_vina_docked_pose_checked.pt")
bfn_p2m_path = os.path.join(result_dir, "molcraft_small_vina_docked_pose_checked.pt")
bfn_dcmpo_path = os.path.join(result_dir, "molcraft_large_vina_docked_pose_checked.pt")

ours_prior_path = os.path.join(result_dir, "ours_prior.pt")
steps80_success_path = os.path.join(result_dir,"steps80_success.pt")
continuous_baseline_path = os.path.join(result_dir, 'continuous_baseline.pt')
success_ours_path = os.path.join(result_dir, 'success_ours.pt')
if __name__ == "__main__":
    ...
    ours_prior = ModelResults("ours_prior", ours_prior_path)
    steps80_success = ModelResults("steps80",steps80_success_path)
    success_ours = ModelResults("success_ours",success_ours_path)
    continuous_baseline = ModelResults('continuous_baseline',
    ...
    # tmp_success = bfn_p2m.is_binding_success()
    ...
    # model = mix_beta08
    # model = continuous_baseline
    model = steps80_success
    ...
    print(np.mean(BF))
    SR = model.is_dcmpdiff_success(qed_list, sa_list, vina_dock_list)
    print(np.mean(SR))
    exit(0)
    ...
    for model in models:
        ...
        print(
            f"{model.name}, hb_donor, {donor_list.mean():.2f}, hb_acceptor, {acceptor_list.mean():.2f}, vdw, {vdw_list.mean():.2f}, hydrophobic, {hydrophobic_list.mean():.2f}"
        )
    exit(0)

    # models = [ref, train, ar, p2m, flag, ds_cond, ds_joint, tg, dcmp, dcmp_ref, bd, ip, bfn, bfn_p2m, bfn_dcmpo]
    models = [ref, ar, p2m, flag, ds_cond, ds_joint, tg, dcmp, dcmp_ref, bd, ip, bfn, bfn_p2m, bfn_dcmpo]
    for model in models:
        model.load_pose_checked()
        model.remove_incomplete()
    ...
    for model in models:
        ring_size_ratio, mol_ring_ratio, mol_ring_no, fused_ring_ratio, mol_fused_ring_ratio = (
            model.ring_size_profile
        )
        tmp, tmp2 = [], []
        for s in ring_sizes:
    ...
    # metric_dict = collections.defaultdict(list)
    # for model in models[1:]:
    #     _metrics = eval_bond_angle.eval_bond_angle_profile(ref.bond_angle_profile, model.bond_angle_profile)
